...
of this engine is that you *can* read it. dyx::engine::run brings the
systems up in dependency order, then runs the main loop. Each system is one
small class:

| class | job |
|---|---|
| dyx::window | the win32 window the engine renders into |
| dyx::input | keyboard and mouse state (down / pressed / released) |
| dyx::device | picks a graphics card and creates the directx 12 device |
| dyx::adapter_selection | enumerates and scores all graphics cards |
| dyx::command_queue | submits work to the gpu and synchronizes cpu/gpu |
| dyx::swap_chain | owns the back buffers that end up on screen |
| dyx::descriptor_heap | hands out descriptor slots (texture views) |
| dyx::shader_library | loads compiled shaders (*.cso) from disk |
| dyx::pso_cache | saves/loads pipeline state objects to/from disk |
| dyx::texture_loader | decodes image files / raw pixels and uploads them to the gpu |
| dyx::font | describes a fixed-grid bitmap font atlas for draw_text |
| dyx::renderer_2d | draws textured quads (this is a 2D engine: everything is a quad) |
...
picking one (dyx::adapter_selection). Each adapter gets a score based on
...
dyx::adapter_preference::lowest_score to run on the weakest card, which is
...
turns `.hlsl` files into `.cso` bytecode that dyx::shader_library loads at
startup.
...
bytecode into real gpu instructions, which is slow. dyx::pso_cache wraps
...
dyx::texture_loader uses stb_image, so png/jpg/bmp/tga all work. The gpu
...
one readable function: dyx::texture_loader::create_from_pixels.

## Text

dyx::renderer_2d::draw_text renders with a bitmap font: one atlas texture
...
Daniel Hepper). The dyx::font struct describes the grid; each character
becomes one quad that shows its part of the atlas. Text rendering is just
sprite sheet drawing.

## The frame

dyx::renderer_2d owns the frame loop plumbing. One unit quad lives in gpu
...
fence (dyx::command_queue) to wait until the gpu released a buffer before
...
`engine_settings::sampler_filter` to dyx::texture_filter::nearest for a
