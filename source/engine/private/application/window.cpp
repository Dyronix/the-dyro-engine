...
namespace
{
	constexpr const wchar_t* window_class_name = L"buz_engine_window";
	...
	LRESULT CALLBACK window_procedure(HWND handle, UINT message, WPARAM w_param, LPARAM l_param)
	{
		...
		auto* owner = reinterpret_cast<buz::window*>(GetWindowLongPtrW(handle, GWLP_USERDATA));
		buz::input* input = owner != nullptr ? owner->get_input() : nullptr;
		...
		case WM_LBUTTONUP:
			if (input != nullptr)
			{
				input->on_mouse_button_event(buz::mouse_button::left, message == WM_LBUTTONDOWN);
			}
			return 0;
		...
		case WM_RBUTTONUP:
			if (input != nullptr)
			{
				input->on_mouse_button_event(buz::mouse_button::right, message == WM_RBUTTONDOWN);
			}
			return 0;
		...
		case WM_MBUTTONUP:
			if (input != nullptr)
			{
				input->on_mouse_button_event(buz::mouse_button::middle, message == WM_MBUTTONDOWN);
			}
			return 0;
