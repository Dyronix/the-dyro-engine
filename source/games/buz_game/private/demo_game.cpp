...
namespace
{
	...
	/// buz::noise_2d and texture_loader::create_from_pixels.
	...
	std::shared_ptr<buz::texture> make_circle_sheet(buz::texture_loader& loader, uint32_t frame_size, uint32_t frame_count)
	{
		...
		for (uint32_t frame = 0; frame < frame_count; ++frame)
		{
			...
			for (uint32_t y = 0; y < frame_size; ++y)
			{
				...
				{
					const buz::vec2 offset = { static_cast<float>(x) - center, static_cast<float>(y) - center };
					if (buz::length(offset) > radius)
					{
						continue;
					}
...
//--------------------------------------------------------------
void demo_game::initialize(buz::engine& engine)
{
...
void demo_game::update(buz::engine& engine, float delta_seconds)
{
	...
	m_ball_position = buz::clamp(m_ball_position, buz::vec2(60.0f, 60.0f), buz::vec2(1220.0f, 660.0f));

	// The mouse wheel resizes the ball
	m_ball_size = buz::clamp(m_ball_size + input.get_mouse_wheel_delta() * 10.0f, 40.0f, 300.0f);

	// Space gives the ball a new random tint
	if (input.was_key_pressed(buz::key::space))
	{
		m_ball_tint = { buz::random_float(), buz::random_float(), buz::random_float(), 1.0f };
		buz::log::info("New ball tint: ({:.2f}, {:.2f}, {:.2f})", m_ball_tint.r, m_ball_tint.g, m_ball_tint.b);
	}
...
//--------------------------------------------------------------
void demo_game::draw(buz::engine& engine, buz::renderer_2d& renderer)
{
	buz::input& input = engine.get_input();
	const buz::vec2 mouse = input.get_mouse_position();
	...
	const buz::rect frame_rect = { { static_cast<float>(frame) * 64.0f, 0.0f }, { static_cast<float>(frame + 1) * 64.0f, 64.0f } };
	...
	const buz::color outline_color = mouse_on_ball ? buz::color{ 1.0f, 0.2f, 0.2f, 1.0f } : buz::color{ 0.2f, 1.0f, 0.2f, 0.6f };
