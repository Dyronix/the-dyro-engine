...
namespace
{
	...
	/// dyro::noise_2d and texture_loader::create_from_pixels.
	...
	std::shared_ptr<dyro::texture> make_circle_sheet(dyro::texture_loader& loader, uint32_t frame_size, uint32_t frame_count)
	{
		...
		for (uint32_t frame = 0; frame < frame_count; ++frame)
		{
			...
			for (uint32_t y = 0; y < frame_size; ++y)
			{
				...
				{
					const dyro::vec2 offset = { static_cast<float>(x) - center, static_cast<float>(y) - center };
					if (dyro::length(offset) > radius)
					{
						continue;
					}
...
void demo_game::initialize(dyro::engine& engine)
{
	...
	const std::filesystem::path content_directory = dyro::paths::get_executable_directory() / "content";
...
void demo_game::update(float delta_seconds)
{
	...
	m_ball_position = dyro::clamp(m_ball_position, dyro::vec2(60.0f, 60.0f), dyro::vec2(1220.0f, 660.0f));

	// The mouse wheel resizes the ball
	m_ball_size = dyro::clamp(m_ball_size + input.get_mouse_wheel_delta() * 10.0f, 40.0f, 300.0f);

	// Space gives the ball a new random tint
	if (input.was_key_pressed(dyro::key::space))
	{
		m_ball_tint = { dyro::random_float(), dyro::random_float(), dyro::random_float(), 1.0f };
		dyro::log::info("New ball tint: ({:.2f}, {:.2f}, {:.2f})", m_ball_tint.r, m_ball_tint.g, m_ball_tint.b);
	}
...
{
	dyro::input& input = m_engine->get_input();
	const dyro::vec2 mouse = input.get_mouse_position();
	...
	const dyro::rect frame_rect = { { static_cast<float>(frame) * 64.0f, 0.0f }, { static_cast<float>(frame + 1) * 64.0f, 64.0f } };
	...
	const dyro::color outline_color = mouse_on_ball ? dyro::color{ 1.0f, 0.2f, 0.2f, 1.0f } : dyro::color{ 0.2f, 1.0f, 0.2f, 0.6f };
